...
REGISTER_MAP_WITH_PROGRAMSTATE(CtorMap, const MemRegion*, const Decl *)
...
bool VirtualCallChecker::isObject(const CallExpr *CE,ProgramStateRef state,const LocationContext *LCtx) const {
  ...
  Expr *base = CME->getBase()->IgnoreImpCasts();
  if (!isa<CXXThisExpr>(base)) {
   ...
   // if (!isa<CXXThisRegion>(reg))
    ...
    for (const auto Reg : RegionMap) {
      if (CEV != state->getSVal(Reg.first))
        return true;
    }
  }
  return false;
