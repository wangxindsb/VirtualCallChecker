...
void VirtualCallChecker::checkPreCall(const CallEvent &Call, 
  ...
  // Enter a object, increase the corresponding integer
  ...
  if (isVirtualCall(CE) && state->get<ConstructorFlag>() > 0 && state->get<ObjectFlag>() == 0) {
    if (!BT_CT) {
  ...
  if (isVirtualCall(CE) && state->get<DestructorFlag>() > 0 && state->get<ObjectFlag>() == 0) {
    if (!BT_DT) {
