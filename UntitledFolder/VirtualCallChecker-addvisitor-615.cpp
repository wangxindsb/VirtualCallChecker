...
void VirtualCallChecker::checkPreCall(const CallEvent &Call, 
  ...
  if (isVirtualCall(CE) && state->get<ConstructorFlag>() > 0) {
    ...
    Reporter->addVisitor(llvm::make_unique<VirtualBugVisitor>(state->get<ConstructorFlag>()));
  ...
  if (isVirtualCall(CE) && state->get<DestructorFlag>() > 0) {
    ...
    Reporter->addVisitor(llvm::make_unique<VirtualBugVisitor>(state->get<DestructorFlag>()));
    C.emitReport(std::move(Reporter));
    return;
  }
}
